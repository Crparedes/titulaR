AmbiDensAireUI <- function(id) {
  ...
  tagList(
    ...
    SiRealInputUI(ns('Temperatura'), name = ReqField('Temperatura'), 20, 2, TemperatureUnits, colWid = c(2, 2, 2, 4)),
    tags$hr(),
    SiRealInputUI(ns('PressionBar'), name = ReqField('Presión barométrica'), 750, 3, AtmosPressuUnits, colWid = c(2, 2, 2, 4)),
    tags$hr(),
    SiRealInputUI(ns('HumedadRela'), name = ReqField('Humedad relativa'), 45, 3, RelatiHumidUnits, colWid = c(2, 2, 2, 4)),
    tags$hr(), uiOutput(ns('NiceDensitAir')),
  )
}
